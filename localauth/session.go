...
type SessionStore interface {
	...
	//
	// # ⛔ 实现约束 (DDL 级, 缺一条本方法就实现不出来)
	//
	//  1. 表里必须有【上一个哈希】列 (prev_hash): 轮换时把当前哈希挪进去再覆盖。
	//     没有它 RotateReplayed 不可能成立 —— 重放检测形同虚设。
	//  2. 轮换必须是【单条原子语句】(UPDATE ... WHERE hash = old AND valid),
	//     并发拿同一个旧哈希只能有一方成功。
	//  3. 失配时按 prev_hash / 当前哈希反查归属, 把 UserID 填进返回的记录。
	//
