...
//
// ⚠️ 分歧点【随数据库而变】, 不要凭一个环境的自查下结论。两处真实存储实测:
//
//	存储   用户名列的排序规则   服务器默认值      忘写 COLLATE 的后果
//	A      大小写不敏感        另一条不敏感规则   分歧在 ß (两边都折叠大小写)
//	B      大小写不敏感        逐字节规则        逐字节比 → 每个大小写变体一个桶
//
// 也就是说「在本地拿 alice/ALICE 试了一下没问题」在生产上恰好是最严重的那种漏。
// 见证串必须从当前库的实际排序规则推导 —— 由宿主的存储层按自己的库算。
//
...
// 实测中见过的形态: 用户名列与服务器默认值是【两条不同的不敏感规则】——
// 不写 COLLATE 就会对不上, 而两者后缀相同, 光看名字看不出来。
//
// # 键含用户提交的原文
//
